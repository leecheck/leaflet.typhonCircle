(function () {
    ...
    L.MarkerMenu = (L.Layer ? L.Layer : L.Class).extend({
        ...
        _initLayout: function () {
            ...
            this._menuTitle = L.DomUtil.create('div', prefix + '-title', this._container);
            this._menuTitle.style.position = 'absolute';
            this._menuTitle.innerHTML = "title";
            this._menuTitle.style[L.DomUtil.TRANSITION] = 'all ' + (this.options.duration /2 / 1000) + 's';
        },
        ...
        _updateMenu: function () {
            ...
            this._menuTitle.innerHTML = this.options.title;

            this._menuList = L.DomUtil.create('ul', 'marker-menu-item', this._container);
            this._menuItems = [];
            ...
            array_map(this._items, function (item, i, items) {
                ...
                menuItem.innerHTML = "<i class=" + "\"" + item.icon + " ivu-icon-button-menu" +"\"" +"></i>";
                menuItem.style.width = this.options.size[0] + 'px';
                menuItem.style.height = this.options.size[1] + 'px';
                menuItem.style.borderRadius = this.options.size[0]/2 + 'px';
                ...
                L.DomEvent.addListener(menuItem, 'click', item.click, item.prop);
                L.DomEvent.on(menuItem, 'click', L.DomEvent.stopPropagation);
        ...
        _updatePosition: function () {
            ...
            setTimeout(function () {
                array_map(that._menuItems, function (item, i) {
                    var itemLeft = (Math.cos(angle * (i + 2)) * radius),
                        itemBottom = (Math.sin(angle * (i + 2)) * radius) ;
